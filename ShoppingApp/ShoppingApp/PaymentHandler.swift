...
class PaymentHandler: NSObject {
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            ...
            let shippingDelivery = PKShippingMethod(label: "Kargo", amount: NSDecimalNumber(string: "0.00"))
            shippingDelivery.dateComponentsRange = PKDateComponentsRange(start: startComponents, end: endComponents)
            shippingDelivery.detail = "Formalar adresinize gönderildi."
            shippingDelivery.identifier = "Kargo"
    ...
    func startPayment(products: [Product], total: Int, completion: @escaping PaymentCompletionHandler) {
        ...
        let total = PKPaymentSummaryItem(label: "Toplam", amount: NSDecimalNumber(string: "\(total).00"), type: .final)
        paymentSummaryItems.append(total)
        
        let paymentRequest = PKPaymentRequest()
        paymentRequest.paymentSummaryItems = paymentSummaryItems
        paymentRequest.merchantIdentifier = "merchant.io.reel.ShoppingApp"
